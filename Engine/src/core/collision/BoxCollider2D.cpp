...
bool Core::BoxCollider2D::SolveCollision(Collider* other,f32 phy_delta){
    ...
    switch(other->getColliderType()){
        case Core::ColliderType::BOX_2D:
            return SolveWithBoxCollider((Core::BoxCollider2D*)other,other->getId(),phy_delta);
            break;
        case Core::ColliderType::CIRCLE_2D:
            return SolveWithCircleCollider((Core::CircleCollider2D*)other,other->getId(),phy_delta);
            break;
    }
    return return_value;
}
